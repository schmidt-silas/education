 ...
 * Erweiterung der Version aus Teil 4 um natuerliche Ordnung.
 * 
 * @author H.Drachenfels
 * @version 9.1.2023
 */
public final class Datum implements Comparable<Datum> { // implements Comparable<Datum>;
    ...
    public int compareTo(Datum d) {
        int result = Integer.compare(this.jahr, d.jahr);
        if (result == 0) {
            result = Integer.compare(this.monat, d.monat);
            if (result == 0) {
                result = Integer.compare(this.tag, d.tag);
            }
        }
